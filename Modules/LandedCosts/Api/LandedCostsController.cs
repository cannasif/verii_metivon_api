...
public sealed class LandedCostsController(ILandedCostService service, MetivonDbContext db, IWebHostEnvironment environment) : ControllerBase
{
    ...
    {
        var dossier=await db.ImportDossiers.AsNoTracking().Where(x=>x.Id==id).Select(x=>new{x.Id,x.DossierNumber,Status=x.Status.ToString(),x.BranchId,x.SupplierId,Supplier=x.Supplier.Name,x.CurrencyCode,x.IncotermCode,x.CustomsDeclarationNumber,x.CustomsDeclarationDate,x.TransactionExchangeRate,x.CustomsExchangeRate,x.CostingExchangeRate,x.OpenDate,x.EstimatedArrivalDate,x.ActualArrivalDate,x.FinalizedAt,x.AllocationRevision,x.Notes}).FirstOrDefaultAsync(ct);
        ...
        return Ok(ApiResponse<object>.Ok(new{dossier,lines,costs,documents,timeline,totals=new{Goods=lines.Sum(x=>x.GoodsAmountLocal),Costs=costs.Sum(x=>x.LocalAmount),Final=lines.Sum(x=>x.GoodsAmountLocal+x.AllocatedCostLocal)}}));
    }
