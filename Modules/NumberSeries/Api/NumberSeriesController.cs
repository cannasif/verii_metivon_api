...
public sealed class NumberSeriesController(INumberSeriesService service) : ControllerBase
{
    ...
    [HttpPut("{id:long}")] public async Task<IActionResult> Update(long id,SaveNumberSeriesRequest request,CancellationToken ct){var result=await service.SaveAsync(id,request,ct);return StatusCode(result.StatusCode,result);}
    [HttpDelete("{id:long}")] public async Task<IActionResult> Delete(long id,CancellationToken ct){var result=await service.DeleteAsync(id,ct);return StatusCode(result.StatusCode,result);}
