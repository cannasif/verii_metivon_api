...
{
    [HttpGet,AllowAnonymous] public async Task<IActionResult> GetActive(CancellationToken ct)=>Ok(ApiResponse<IReadOnlyList<BranchItem>>.Ok(await service.GetActiveAsync(ct)));
    ...
    [HttpPost("{id:long}/update"),PermissionAuthorize("organization.branches.update")] public async Task<IActionResult> Update(long id,SaveBranchRequest request,CancellationToken ct){var result=await service.SaveAsync(id,request,ct);return StatusCode(result.StatusCode,result);}
    [HttpPost("{id:long}/delete"),PermissionAuthorize("organization.branches.delete")] public async Task<IActionResult> Delete(long id,CancellationToken ct){var result=await service.DeleteAsync(id,ct);return StatusCode(result.StatusCode,result);}
}
