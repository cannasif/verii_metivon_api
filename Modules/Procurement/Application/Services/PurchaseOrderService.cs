...
public sealed class PurchaseOrderService(IUnitOfWork u,IProcurementParameterService parameters,MetivonDbContext db,INumberSeriesService numberSeries):IPurchaseOrderService
{
 ...
 public async Task<ApiResponse<object>>CreateAsync(CreatePurchaseOrderRequest r,CancellationToken ct)
 {
  ...
  ReservedNumber? reserved=null;string number;try{if(string.IsNullOrWhiteSpace(r.OrderNumber)){reserved=await numberSeries.ReserveAsync(new("Procurement","PurchaseOrderNumber",null,r.BranchId,r.WarehouseId,null,r.SupplierId,null,null,r.OrderDate,"PurchaseOrder"),ct);number=reserved.DocumentNumber;}else number=await parameters.ResolveOrderNumberAsync(r.OrderNumber,r.BranchId,ct);}catch(InvalidOperationException exception){return ApiResponse<object>.Error(exception.Message,400);}
  ...
  await u.ExecuteInTransactionAsync(async t=>{await u.Repository<PurchaseOrder>().AddAsync(e,t);await u.SaveChangesAsync(t);if(trade is not null){await u.Repository<TradeDocumentLink>().AddAsync(new(){TradeDossierId=trade.Id,LinkType=TradeLinkType.PurchaseOrder,SourceId=e.Id,ReferenceNumber=e.OrderNumber},t);await u.SaveChangesAsync(t);}},ct);if(reserved is not null)await numberSeries.MarkUsedAsync(reserved.UsageId,e.Id,ct);return ApiResponse<object>.Ok(new{e.Id,e.OrderNumber,e.Status,e.TradeDossierId,NumberSeriesUsageId=reserved?.UsageId});
 }
