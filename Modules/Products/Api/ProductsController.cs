...
public sealed class ProductsController(IProductService service) : ControllerBase
{
    ...
    [HttpPost("definition-management/{kind}/{id:long}/update")] public async Task<IActionResult> UpdateDefinition(string kind,long id,SaveProductDefinitionRequest request,CancellationToken ct){var response=await service.UpdateDefinitionAsync(kind,id,request,Culture,ct);return StatusCode(response.StatusCode,response);}
    [HttpPost("definition-management/{kind}/{id:long}/delete")] public async Task<IActionResult> DeleteDefinition(string kind,long id,CancellationToken ct){var response=await service.DeleteDefinitionAsync(kind,id,Culture,ct);return StatusCode(response.StatusCode,response);}
}
