...
public sealed class ProductService(IUnitOfWork unitOfWork, IProductParameterService parameters) : IProductService
{
    ...
    private async Task<ApiResponse<object>> DeactivateDefinition<T>(IGenericRepository<T> repository,long id,CancellationToken ct)where T:DefinitionEntity{var entity=await repository.GetByIdForUpdateAsync(id,ct);if(entity is null)return ApiResponse<object>.Error("Definition not found.",404);if(entity.IsDefault)return ApiResponse<object>.Error("Default definition cannot be deleted.",409);entity.IsActive=false;repository.Update(entity);await unitOfWork.SaveChangesAsync(ct);return ApiResponse<object>.Ok(new{entity.Id});}
