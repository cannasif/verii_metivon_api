...
public sealed class GoodsReceiptPurchaseOrderConfiguration:IEntityTypeConfiguration<GoodsReceiptPurchaseOrder>{public void Configure(EntityTypeBuilder<GoodsReceiptPurchaseOrder>b){b.ToTable("RII_GOODS_RECEIPT_PURCHASE_ORDERS");b.HasKey(x=>x.Id);b.HasQueryFilter(x=>!x.IsDeleted);b.HasIndex(x=>new{x.GoodsReceiptId,x.PurchaseOrderId}).IsUnique().HasFilter("[IsDeleted] = 0");b.HasOne(x=>x.GoodsReceipt).WithMany(x=>x.PurchaseOrders).HasForeignKey(x=>x.GoodsReceiptId).OnDelete(DeleteBehavior.Restrict);b.HasOne(x=>x.PurchaseOrder).WithMany().HasForeignKey(x=>x.PurchaseOrderId).OnDelete(DeleteBehavior.Restrict);}}
